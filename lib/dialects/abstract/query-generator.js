			...
			if (Object.prototype.hasOwnProperty.call(valueHash, key)) {
				...
				if (
					...
					if (value instanceof Utils.SequelizeMethod || options.bindParam === false) {
						values.push(
							this.escape(value, (modelAttributeMap && modelAttributeMap[key]) || undefined, {
								context: 'INSERT',
							})
						);
					} else {
		...
		switch (options.type.toUpperCase()) {
			...
			case 'FOREIGN KEY':
				...
				constraintName = this.quoteIdentifier(
					options.name || `${tableName}_${fieldsSqlString}_${references.table}_fk`
				);
				...
				if (previousModel && previousModel.prototype instanceof Model) {
					...
					if (model) {
						...
						if (!(item instanceof Association)) {
							throw new Error(
								util.format("Unable to find a valid association for model, '%s'", model.name)
							);
						}
					}
				}
		...
		if (topLevelInfo.options.includeIgnoreAttributes !== false) {
			...
			const includeAttributes = include.attributes.map((attr) => {
				...
				if (Array.isArray(attr) && attr.length === 2) {
					...
					attr = attr.map((attr) =>
						attr instanceof Utils.SequelizeMethod ? this.handleSequelizeMethod(attr) : attr
					);
					...
					where: {
						[Op.and]: [
							this.sequelize.literal(
								[
									`${this.quoteTable(topParent.model.name)}.${this.quoteIdentifier(
