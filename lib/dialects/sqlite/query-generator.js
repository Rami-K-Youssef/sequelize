...
class SQLiteQueryGenerator extends MySqlQueryGenerator {
	...
	updateQuery(tableName, attrValueHash, where, options, attributes) {
		...
		if (options.limit) {
			...
			query = `UPDATE ${this.quoteTable(tableName)} SET ${values.join(',')} ${this.whereQuery(
				where,
				whereOptions
			)}`;
		}
	...
	truncateTableQuery(tableName, options = {}) {
		return [
			...
			options.restartIdentity
				? `; DELETE FROM ${this.quoteTable('sqlite_sequence')} WHERE ${this.quoteIdentifier(
						'name'
				  )} = ${Utils.addTicks(Utils.removeTicks(this.quoteTable(tableName), '`'), "'")};`
				: '',
		].join('');
	}
