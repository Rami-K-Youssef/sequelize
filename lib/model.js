			...
			if (options.alter === true || (typeof options.alter === 'object' && options.alter.drop !== false)) {
				for (const columnName in columns) {
					...
					if (typeof this.queryInterface.queryGenerator.escape == 'function') {
						options.escape = this.queryInterface.queryGenerator.escape.bind(this.queryInterface.queryGenerator)
					}

					let columnSqlType = currentAttribute.type.toSql(options).replace('INTEGER','INT');
