...
class Model {
	...
	static init(attributes, options = {}) {
		...
		options = Utils.merge(
			{
				name: {
					plural: Utils.pluralize(_.camelCase(options.modelName)),
					singular: Utils.singularize(_.camelCase(options.modelName)),
				},
				indexes: [],
		...
		_.each(this.rawAttributes, (definition, name) => {
			...
			if (Object.prototype.hasOwnProperty.call(definition, 'defaultValue')) {
				this._defaultValues[name] = () =>
					Utils.toDefaultValue(definition.defaultValue, this.sequelize.options.dialect);
			}
	...
	static async sync(options) {
		...
		Object.values(attributes).forEach((attribute) => {
			if (_.isObject(attribute.references) && attribute.constraintName == null) {
				attribute.constraintName = `FK_${this.tableName}_${attribute.field}`;
			}
		});
		...
		if (options.alter) {
			...
			//remove all non existing constraints
			const constraintNames = foreignKeyReferences.map((item) => item.constraint_name || item.constraintName);
			const foundConstraintNames = [];
			Object.values(rawAttributes).forEach((attr) => {
				if (attr.references && attr.constraintName) foundConstraintNames.push(attr.constraintName);
			});
			...
			if (options.alter === true || (typeof options.alter === 'object' && options.alter.drop !== false)) {
				for (const columnName in columns) {
					...
					if (currentAttribute.primaryKey && !currentAttribute.references) continue;
					// Check foreign keys. If it's a foreign key, it should remove constraint first.
					const references = currentAttribute.references;

					const toSqlOptions = {};

					if (typeof this.queryInterface.queryGenerator.escape == 'function') {
						toSqlOptions.escape = this.queryInterface.queryGenerator.escape.bind(this.queryInterface.queryGenerator)
					}

					let myType = currentAttribute.type.toSql(toSqlOptions);
					let dbType = column.type;

					myType = myType.replace('INTEGER', 'INT');
					const numericTypes = ['INT', 'TINYINT', 'TINYINT', 'SMALLINT', 'BIGINT'];
					if (numericTypes.some((type) => dbType.startsWith(type))) {
						//strip Width
						dbType = dbType.split(/\(\d+\)/).join('');
						myType = myType.split(/\(\d+\)/).join('');
					}

					let currentAttributeDefaultValue = currentAttribute.defaultValue;
					if (Buffer.isBuffer(currentAttributeDefaultValue)) {currentAttributeDefaultValue = '0x' + currentAttributeDefaultValue.toString('hex'); if (currentAttributeDefaultValue == '0x00') currentAttributeDefaultValue = '0x';}
					if (typeof currentAttributeDefaultValue == 'boolean') currentAttributeDefaultValue = currentAttributeDefaultValue ? '1' : '0';

					changeFlag = !(column.type == columnSqlType
					&& column.allowNull == (currentAttribute.allowNull == undefined ? true : currentAttribute.allowNull)
					&& column.defaultValue == (currentAttributeDefaultValue?.toString() || null));
					...
					if (currentAttribute.references) {
						...
						for (const foreignKeyReference of foreignKeyReferences) {
							...
							if (
								!!constraintName &&
								currentAttribute.constraintName == constraintName &&
								...
								if (!changeFlag){
									// console.warn(column);
									// console.debug(foreignKeyReference);
									// console.debug(currentAttribute);
									
									changeFlag = !(foreignKeyReference.deleteRule == currentAttribute.onDelete.toString().toUpperCase()
										&& foreignKeyReference.updateRule == currentAttribute.onUpdate.toString().toUpperCase()
										&& currentAttribute.constraintName == foreignKeyReference.constraintName
										&& currentAttribute.references.model == foreignKeyReference.referencedTableName
										&& currentAttribute.references.key == foreignKeyReference.referencedColumnName
									);
					...
					if (changeFlag) await this.queryInterface.changeColumn(tableName, columnName, currentAttribute, options);
				}
			}
			await Promise.all(
				constraintNames
					.filter((item) => !foundConstraintNames.includes(item))
					.map(async (constraintName) => {
						try {
							await this.queryInterface.removeConstraint(tableName, constraintName, options);
						} catch {}
					})
									...
									if (instance[include.association.through.model.name]) {
										for (const attr of Object.keys(
											...
											if (
												...
												attr === include.association.foreignKey ||
												attr === include.association.otherKey ||
												typeof instance[include.association.through.model.name][attr] ===
													undefined
											) {
												continue;
											}
	 ...
	 * @param {string}              [options.constraintName] Defaults to FK_SourceTableName_ForeignKeyColumnName
