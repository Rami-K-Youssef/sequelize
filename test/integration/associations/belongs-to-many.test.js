...
describe(Support.getTestDialectTeaser('BelongsToMany'), () => {
	...
	describe('through model validations', () => {
		...
		it('runs on create', async function () {
			await expect(this.project.createParticipant({ name: 'employee 2' }, { through: { role: '' } })).to.be
				.rejected;
		});
	});
	...
	describe('through', () => {
		describe('paranoid', () => {
			...
			it('hasAssociation also respects paranoid option', async function () {
				...
				expect(await this.users[0].hasProjects(this.projects[0], { through: { paranoid: false } })).to.equal(
					true
				);
