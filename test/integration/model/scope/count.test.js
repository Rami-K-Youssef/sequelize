	...
	describe('scope', () => {
		describe('count', () => {
			...
			it('should be able to merge scopes with where', async function () {
				await expect(this.ScopeMe.scope('lowAccess').count({ where: { username: 'dan' } })).to.eventually.equal(
					1
				);
			});
			...
			it('should be able to use include with function scope and string association', async function () {
				await expect(this.ScopeMe.scope('withIncludeFunctionAndStringAssociation').count()).to.eventually.equal(
					1
				);
			});
		});
