...
describe(Support.getTestDialectTeaser('Model'), () => {
	...
	describe('create', () => {
		...
		it('works with custom timestamps with a default value', async function () {
			...
			expect([user1.created_time.getMilliseconds(), user2.created_time.getMilliseconds()]).not.to.deep.equal([
				0,
				0,
			]);
			expect([user1.updated_time.getMilliseconds(), user2.updated_time.getMilliseconds()]).not.to.deep.equal([
				0,
				0,
			]);
		});
