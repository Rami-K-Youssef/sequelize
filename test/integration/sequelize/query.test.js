	...
	describe('query', () => {
		...
		beforeEach(async function () {
			...
			this.insertQuery = `INSERT INTO ${qq(this.User.tableName)} (username, email_address, ${qq(
				'createdAt'
			)}, ${qq('updatedAt')}) VALUES ('john', 'john@gmail.com', '2012-01-01 10:10:10', '2012-01-01 10:10:10')`;
		...
		describe('rejections', () => {
			...
			it('reject if `sql.bind` and `sql.values` are both passed', async function () {
				await this.sequelize
					.query(
						{ query: 'select $1 + ? as foo, $2 + ? as bar', bind: [1, 2], values: [1, 2] },
						{ raw: true }
					)
					.should.be.rejectedWith(Error, 'Both `replacements` and `bind` cannot be set at the same time');
			});
		...
		it('dot separated attributes when doing a raw query without nest', async function () {
			...
			await expect(
				this.sequelize.query(sql, { raw: true, nest: false }).then((obj) => obj[0])
		...
		it('escape where has $ on the middle of characters', async function () {
			const typeCast = dialect === 'postgres' ? '::int' : '';
			const result = await this.sequelize.query(`select $one${typeCast} as foo$bar`, {
				raw: true,
				bind: { one: 1 },
			});
