...
if (current.dialect.name === 'mssql') {
	describe('[MSSQL Specific] QueryGenerator', () => {
		...
		it('createDatabaseQuery with collate', function () {
			expectsql(
				this.queryGenerator.createDatabaseQuery('myDatabase', { collate: 'Latin1_General_CS_AS_KS_WS' }),
				{
					mssql:
		...
		it('createTableQuery with comments', function () {
			expectsql(
				this.queryGenerator.createTableQuery(
					'myTable',
					{ int: 'INTEGER COMMENT Foo Bar', varchar: 'VARCHAR(50) UNIQUE COMMENT Bar Foo' },
					{}
				),
				{
					mssql:
		...
		it('dropConstraintQuery', function () {
			expectsql(
				this.queryGenerator.dropConstraintQuery({ tableName: 'myTable', schema: 'mySchema' }, 'myConstraint'),
				{
