		...
		if (Support.getTestDialect() === 'postgres') {
			...
			describe('pgEnumName', () => {
				it('does not add schema when options: { schema: false }', () => {
					expect(sql.pgEnumName(PublicUser.getTableName(), 'mood', { schema: false })).to.equal(
						'"enum_users_mood"'
					);
				...
				it('properly quotes both the schema and the enum name', () => {
					expect(
						sql.pgEnumName(PublicUser.getTableName(), 'mood', PublicUser.rawAttributes.mood.type)
					).to.equal('"public"."enum_users_mood"');
					expect(
						sql.pgEnumName(FooUser.getTableName(), 'theirMood', FooUser.rawAttributes.mood.type)
